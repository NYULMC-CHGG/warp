...
workflow Glimpse2SplitReference {
    ...
    scatter (i_contig in range(length(contig_regions))) {
        ...
        String split_ref_name = reference_panel_prefix+ contig_name_in_reference_panel + split_panel_suffix
        String split_ref_index = split_ref_name + reference_panel_index_suffix
        ...
        call GlimpseSplitReferenceTask {
            input:
                ...
                split_ref = split_ref_name,
                split_ref_index = split_ref_index,
...
task GlimpseSplitReferenceTask {
    ...
    command <<<
        ...
        do
            ...
            /bin/GLIMPSE2_split_reference --threads ${NPROC} --reference ~{split_ref} --map ~{genetic_map} --input-region ${IRG} --output-region ${ORG} --output ~{reference_output_dir}/reference_panel_contigindex_${CONTIGINDEX}_chunkindex_${CHUNKINDEX} ~{"--seed "+seed}
